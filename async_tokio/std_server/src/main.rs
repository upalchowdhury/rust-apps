...
fn main() {
    ...
    // starting
    println!("STD starting {}", STD_SERVER_ADDRESS);

    // adding a delay
    println!("STD has a delay of {} milliseconds", delay);
    
    // bind
    let listener = TcpListener::bind(STD_SERVER_ADDRESS).unwrap();

    // starting
    println!("STD listening {}", STD_SERVER_ADDRESS);
